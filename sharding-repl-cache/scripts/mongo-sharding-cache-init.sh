...
docker compose exec -T mongo-config-srv-cache mongosh --port 27117 --quiet <<MONGO
rs.initiate({
  _id: "mongo-config-srv-cache",
  configsvr: true,
  members: [
    { _id: 0, host: "mongo-config-srv-cache:27117" }
  ]
});
MONGO
...
docker compose exec -T mongo-router-cache mongosh --port 27120 --quiet <<MONGO
